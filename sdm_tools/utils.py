...
def get_local_timezone(tz_string=None):
    ...
    try:
        ...
        console.print(
            f"[bold yellow]Warning: Invalid timezone '{tz_string}', using UTC: {e}[/bold yellow]"
        )
        return ZoneInfo("UTC")
...
def parse_git_date_to_local(date_str, target_tz=None):
    ...
    try:
        ...
        console.print(
            f"[bold yellow]Warning: Could not parse git date '{date_str}' with timezone, assuming UTC: {e}[/bold yellow]"
        )
        try:
            ...
            console.print(
                f"[bold red]Error: Failed to parse git date '{date_str}': {fallback_e}[/bold red]"
            )
            return None
...
def get_off_hours_window(date_obj, tz=None):
    ...
    end = datetime.combine(date_obj, datetime.min.time()).replace(
        hour=7, minute=59, second=59, microsecond=999999, tzinfo=tz
    )
