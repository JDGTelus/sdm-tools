...
def calculate_sprint_velocity(sprint_id=None, limit=10):
    ...
    try:
        ...
        if "story_points" not in columns:
            console.print(
                "[bold red]Error: story_points column not found in issues table.[/bold red]"
            )
            console.print("[yellow]The database schema needs to be updated.[/yellow]")
            console.print(
                "[yellow]Please run a full data refresh to add the story_points column:[/yellow]"
            )
            ...
            console.print(
                "[dim]This will backup your current database and create a new schema with story points support.[/dim]"
            )
            return []
        ...
        full_where = f"{base_where} AND s.id = {sprint_id}" if sprint_id else base_where
        ...
        if "no such column" in str(e).lower():
            ...
            console.print(
                '[cyan]  2. Choose option 1: "Refresh All Data (Jira + Git → Normalize)"[/cyan]'
            )
            console.print('[cyan]  3. Confirm with "yes"[/cyan]')
            console.print(
                "[dim]This will add the story_points column and populate it with data from Jira.[/dim]"
            )
        else:
            console.print(f"[bold red]Database Error: {str(e)}[/bold red]")
        return []
...
def get_sprint_velocity_summary(limit=10):
    ...
    return {
        ...
        "overall_completion_rate": (
            round((total_delivered / total_planned * 100), 1) if total_planned > 0 else 0.0
        ),
    }
...
def display_sprint_velocity(limit=10):
    ...
    for sprint in sprints:
        ...
        completion_color = (
            "green"
            if sprint["completion_rate"] >= 80
            else "yellow" if sprint["completion_rate"] >= 60 else "red"
        )
