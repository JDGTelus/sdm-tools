...
def ingest_jira_issue(conn, issue_data):
    ...
    try:
        ...
        console.print(f"[yellow]Warning: Error ingesting issue {issue_data.get('key')}: {e}[/yellow]")
        return 0
...
def ingest_git_commit(conn, commit_data):
    ...
    try:
        ...
        console.print(f"[yellow]Warning: Error ingesting commit {commit_data.get('hash')}: {e}[/yellow]")
        return False
...
def calculate_sprint_points(conn):
    ...
    for sprint_name in sprints:
        ...
        for issue_key, metadata_json in cursor.fetchall():
