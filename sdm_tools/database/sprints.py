...
def extract_sprint_data_from_issues():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor = execute_sql(
            conn, f"SELECT name FROM sqlite_master WHERE type='table' AND name='{TABLE_NAME}'")
        ...
        cursor = execute_sql(
            conn, f"SELECT customfield_10020 FROM {TABLE_NAME} WHERE customfield_10020 IS NOT NULL AND customfield_10020 != ''")
...
def create_sprints_table(sprints_data):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor = execute_sql(
            conn, f"SELECT name FROM sqlite_master WHERE type='table' AND name='{sprint_table_name}'")
        if cursor.fetchone():
        ...
        for sprint_id, sprint in sprints_data.items():
            ...
            values = [sprint_id] + [str(sprint.get(field, ''))
...
def process_sprints_from_issues():
    ...
    console.print(
        "[bold yellow]Extracting sprint data from issues...[/bold yellow]")
    ...
    console.print(
        f"[bold cyan]Found {len(sprints_data)} unique sprints.[/bold cyan]")
    ...
    if success:
        ...
        display_choice = console.input(
            "[bold yellow]Do you want to display the sprints table? (y/N): [/bold yellow]").strip().lower()
