...
def ensure_database_exists():
    ...
    if not db_exists:
        ...
        if not cursor.fetchone():
            console.print(f"[yellow]Database exists but schema is missing. Creating schema...[/yellow]")
            create_simple_schema(conn)
...
def refresh_data(full_refresh=False):
    ...
    """
    console.print("\n[bold cyan]═══════════════════════════════════════════════[/bold cyan]")
    console.print(f"[bold cyan]    DATA REFRESH ({'FULL' if full_refresh else 'INCREMENTAL'})[/bold cyan]")
    ...
    try:
        ...
        console.print(f"[green]  ✓ Sprint points calculated[/green]")
    ...
    try:
        if full_refresh:
            ...
            if last_hash:
                ...
                cmd = ["git", "log", f"{last_hash}..HEAD", "--all", "--pretty=format:%H|%an|%ae|%aI|%s"]
            else:
