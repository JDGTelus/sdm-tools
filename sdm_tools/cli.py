...
def handle_sprint_analytics_option():
    ...
    if os.path.exists(sprint_stats_file):
        ...
        update_choice = (
            console.input(
                "[bold yellow]Sprint analytics file already exists. Do you want to update it? (y/N): [/bold yellow]"
            )
            .strip()
            .lower()
        )
...
def handle_developer_activity_option():
    ...
    if os.path.exists(activity_file):
        ...
        update_choice = (
            console.input(
                "[bold yellow]Developer activity file already exists. Do you want to update it? (y/N): [/bold yellow]"
            )
            .strip()
            .lower()
        )
...
def handle_html_generation_option():
    ...
    try:
        ...
        if not os.path.exists(dist_dir):
            ...
            console.print(f"[bold green]Created {dist_dir} directory.[/bold green]")

        # Read the CSS file once
        css_file = "ux/web/shared-dashboard-styles.css"
        if not os.path.exists(css_file):
            console.print(f"[bold red]CSS file not found at {css_file}.[/bold red]")
            input("Press Enter to return to the menu...")
            return
        ...
        for html_file in html_files:
            try:
                ...
                console.print(f"[bold cyan]Processing {filename}...[/bold cyan]")
                ...
                # For comparison dashboard, handle Promise.all differently
                if "comparison" in filename.lower():
                    # Load activity data as well
                    activity_file = "ux/web/data/developer_activity.json"
                    if os.path.exists(activity_file):
                        with open(activity_file, "r", encoding="utf-8") as f:
                            activity_json = json.load(f)

                        # Find Promise.all pattern
                        promise_start = html_content.find("Promise.all([")
                        if promise_start != -1:
                            useEffect_start = html_content.rfind(
                                "useEffect(() => {", 0, promise_start
                            )
                            if useEffect_start != -1:
                                ...
                                if useEffect_end != -1:
                                    ...
                                    sprint_json_str = json.dumps(json_data, indent=8)
                                    activity_json_str = json.dumps(
                                        activity_json, indent=8
                                    )

                                    new_useEffect = f"""useEffect(() => {{
                const sprintData = {sprint_json_str};
                const activityData = {activity_json_str};
                ...
                console.print(f"[bold green]✓ Generated: {target_file}[/bold green]")
