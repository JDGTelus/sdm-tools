...
class TestMergeDeveloperData:
    ...
    def test_merge_jira_only_developer(self):
        ...
        jira_devs = {
            "john.doe@example.com": {"name": "John Doe", "account_id": "acc-123", "aliases": set()}
        }
        git_emails = {}
    ...
    def test_merge_git_only_developer(self):
        ...
        jira_devs = {}
        git_emails = {"jane.smith@example.com": ["jane.smith@example.com", "jsmith@example.com"]}
...
class TestPopulateDevelopersTable:
    ...
    def test_populate_single_developer(self, in_memory_db):
        ...
        developers_data = {
            "test@example.com": {"name": "Test User", "account_id": "acc-123", "aliases": set()}
        }
        ...
        cursor.execute(
            "SELECT email, name, jira_account_id FROM developers WHERE email = ?",
            ("test@example.com",),
        )
        row = cursor.fetchone()
    ...
    def test_populate_developer_with_aliases(self, in_memory_db):
        ...
        cursor = in_memory_db.cursor()
        cursor.execute(
            "SELECT alias_email FROM developer_email_aliases WHERE developer_id = ?", (dev_id,)
        )
...
class TestFindDeveloperIdByEmail:
    ...
    def test_find_with_aws_sso_prefix(self, in_memory_db):
        ...
        found_id = find_developer_id_by_email(
            in_memory_db, "AWSReservedSSO_Role/john@telusinternational.com"
        )
