...
def export_developer_data_to_json(output_file: str = "developer_data.json"):
    ...
    try:
        console.print("[bold blue]🔍 Generating developer analytics data...[/bold blue]")
        ...
        console.print(f"[green]✅ Successfully exported data for {len(metrics)} developers to {output_file}[/green]")
        console.print(f"[cyan]📊 Team Summary:[/cyan]")
        console.print(f"  • Total Commits: {export_data['team_summary']['total_commits']:,}")
        console.print(f"  • Issues Resolved: {export_data['team_summary']['total_issues_resolved']}")
        console.print(f"  • Avg Productivity: {export_data['team_summary']['avg_productivity_score']}/100")
...
def _generate_badges(metric):
    ...
    if metric.productivity_score >= 80:
        badges.append({"name": "High Performer", "color": "green", "icon": "🚀"})
    elif metric.productivity_score >= 60:
        badges.append({"name": "Solid Contributor", "color": "blue", "icon": "💪"})
    ...
    if metric.code_quality_score >= 75:
        badges.append({"name": "Quality Champion", "color": "yellow", "icon": "⭐"})
    
    # Commit frequency badges
    if metric.commit_frequency >= 1.0:
        badges.append({"name": "Daily Committer", "color": "orange", "icon": "📅"})
...
def _generate_developer_insights(metric):
    ...
    if metric.productivity_score >= 80:
        insights.append("Exceptional productivity levels - keep up the great work!")
    elif metric.productivity_score < 40:
        insights.append("Consider focusing on increasing commit frequency and issue resolution.")
    
    # Collaboration insights
    if metric.collaboration_score >= 70:
        insights.append("Strong collaboration skills evident through merge commits and team engagement.")
    elif metric.collaboration_score < 40:
        insights.append("Consider participating more in code reviews and team discussions.")
    ...
    if metric.peak_commit_hour < 9:
        ...
        insights.append("Night owl developer - most productive in evening hours.")
    
    # Issue resolution insights
    if metric.avg_resolution_time > 0:
        if metric.avg_resolution_time < 7:
            insights.append("Quick issue resolution - excellent turnaround time!")
        elif metric.avg_resolution_time > 14:
            insights.append("Consider breaking down complex issues into smaller tasks.")
